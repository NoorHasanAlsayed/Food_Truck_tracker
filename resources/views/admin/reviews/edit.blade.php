    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reviews.update", [$review->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rate') ? 'is-invalid' : '' }}" type="text" name="rate" id="rate" value="{{ old('rate', $review->rate) }}" required>
                @if($errors->has('rate'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('body') ? 'is-invalid' : '' }}" type="text" name="body" id="body" value="{{ old('body', $review->body) }}">
                @if($errors->has('body'))
                    <div class="invalid-feedback">
                        {{ $errors->first('body') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('truck') ? 'is-invalid' : '' }}" name="truck_id" id="truck_id">
                    @foreach($trucks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('truck_id') ? old('truck_id') : $review->truck->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('truck'))
                    <div class="invalid-feedback">
