...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-cuisineFoodTrucks">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($foodTrucks as $key => $foodTruck)
                        <tr data-entry-id="{{ $foodTruck->id }}">
                            ...
                            <td>
                                @can('food_truck_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.food-trucks.show', $foodTruck->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('food_truck_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.food-trucks.edit', $foodTruck->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('food_truck_delete')
                                    <form action="{{ route('admin.food-trucks.destroy', $foodTruck->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-cuisineFoodTrucks:not(.ajaxTable)').DataTable({ buttons: dtButtons })
